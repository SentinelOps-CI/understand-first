...
def scan_boundaries(path: str) -> dict[str, Any]:
    ...
    for root, _dirs, files in os.walk(path):
        for f in files:
            ...
            try:
                head = open(full, encoding="utf-8", errors="ignore").read(2000)
            except OSError:
                continue
            if f.endswith((".yml", ".yaml")) and "openapi" in head.lower():
                data = open(full, encoding="utf-8", errors="ignore").read()
    ...
    return {
        "openapi_paths": sorted(set(endpoints)),
        "proto_rpcs": sorted(set(protos)),
