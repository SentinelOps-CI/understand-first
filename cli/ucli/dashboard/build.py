...
def build_dashboard(paths: dict[str, str]) -> str:
    repo = (
        json.load(open(paths.get("repo", ""), encoding="utf-8"))
        if os.path.exists(paths.get("repo", ""))
        else {}
    )
    lens = (
        json.load(open(paths.get("lens", ""), encoding="utf-8"))
        if os.path.exists(paths.get("lens", ""))
        else {}
    )
    bounds = (
        json.load(open(paths.get("bounds", ""), encoding="utf-8"))
        if os.path.exists(paths.get("bounds", ""))
        else {}
    )
