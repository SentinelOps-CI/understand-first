...
def lens_delta_svg(old_path: str, new_path: str) -> str:
    ...
    for q in sorted(common):
        svg.append(f'<text x="20" y="{y}" font-size="12" fill="#444">{html.escape(q)}</text>')
        y += 18
    for q in sorted(added):
        svg.append(f'<text x="20" y="{y}" font-size="12" fill="#0a0">+ {html.escape(q)}</text>')
        y += 18
    for q in sorted(removed):
        svg.append(f'<text x="20" y="{y}" font-size="12" fill="#a00">- {html.escape(q)}</text>')
        y += 18
    svg.append("</svg>")
    return "\n".join(svg)
