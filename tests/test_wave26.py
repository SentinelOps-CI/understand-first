...
def test_rust_mod_path_cross_module_edge(force_regex):
    ...
    assert any(
        isinstance(c, str) and c.endswith(":other_fn")
        for c in m["functions"][run]["calls"]
    )
...
def test_rust_same_file_baseline_still_works(force_regex):
    ...
    assert any(
        isinstance(c, str) and c.endswith(":add")
        for c in m["functions"][compute]["calls"]
    )


def test_rust_does_not_invent_without_mod_use(
...
def test_rust_omits_ambiguous_cross_module_edges(
    tmp_path: pathlib.Path, force_regex
):
...
def test_rust_omits_when_two_mods_export_same_bare_name(
    tmp_path: pathlib.Path, force_regex
):
...
def test_rust_ast_cross_module_edges(force_ast):
    ...
    assert any(
        isinstance(c, str) and c.endswith(":other_fn")
        for c in m["functions"][run_mod]["calls"]
    )
